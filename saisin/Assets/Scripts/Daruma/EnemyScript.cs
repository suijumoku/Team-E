...
public class EnemyScript : MonoBehaviour
{
    ...
    void GotoNextPoint()
    {
        ...
        if (agent != null && agent.pathStatus != NavMeshPathStatus.PathInvalid)
        {
            ...
            // 準備が整う前？にResume とか SetDestination とかしようとすると発生するエラーがあるので
            // それをチェックするためのif文

            agent.SetDestination(points[nextPoint[nextPointIndex]].position);
        }
        else
            print("erorr");
    ...
    void OnCollisionEnter(Collision collision)
    {
        ...
        if (collision.gameObject.tag == "Hammer" && _playerController.isAttack == true && _playerController.isHit == false)
        {
            ...
            if (gameObject.tag == "Enemy")
            {
                ...
                var r = child.gameObject.GetComponent<EnemyScript>();
                if (r != null)
                    r.enabled = true;
            ...
            {
                gameObject.tag = "DarumaBall"; //打ち直しをできるように
                Force(collision);
            }
